...
const WHY_ICON_PATHS: Record<string, string> = {
  ...
  desktop:
    "M9 17.25v1.007a3 3 0 01-.879 2.122L7.5 21h9l-.621-.621A3 3 0 0115 18.257V17.25m6-12V15a2.25 2.25 0 01-2.25 2.25H5.25A2.25 2.25 0 013 15V5.25m18 0A2.25 2.25 0 0018.75 3H5.25A2.25 2.25 0 003 5.25m18 0h-18",
  users:
...
function accentLastTwoWords(text: string) {
  const words = text.trim().split(/\s+/);
  if (words.length <= 2) {
    return (
      <span className="bg-gradient-to-r from-[#00E1FF] to-[#0055FF] text-transparent bg-clip-text">
...
function resolveCtaBody(industry: IndustryData) {
  const body = industry.ctaBody || "";
  return body
    .replace(/\{title\}/gi, industry.title)
    .replace(/\{industry\}/gi, industry.title);
}
...
export default function IndustryDetailClient({
  ...
  slug: string;
  initialIndustry: IndustryData | null;
  initialNavChrome?: typeof DEFAULT_NAV_CHROME;
}) {
  const [industry, setIndustry] = useState<IndustryData | null>(initialIndustry);
  const [navChrome, setNavChrome] = useState(initialNavChrome || DEFAULT_NAV_CHROME);

  useEffect(() => {
    if (initialIndustry) {
      setIndustry(initialIndustry);
      if (initialNavChrome) setNavChrome(initialNavChrome);
      return;
    }
    let cancelled = false;
    fetch(`/api/cms/industries/${slug}`, { cache: "no-store" })
  ...
  }, [slug, initialIndustry, initialNavChrome]);
        ...
        <main className="min-h-screen bg-white flex items-center justify-center">
          <div className="text-center">
            <h1 className="text-4xl font-bold text-[#0f172a] mb-4">
              {industriesListingDefaults.notFoundHeading}
            </h1>
            <Link href="/industries" className="text-[#0f172a] hover:underline">
              {industriesListingDefaults.notFoundLinkLabel}
            </Link>
          </div>
        </main>
        <Footer />
  ...
  const heroSrc = industry.heroImage
  ...
  const showWhy =
    ...
    Array.isArray(industry.whyChooseUsCards) &&
    industry.whyChooseUsCards.length > 0;
    ...
    <>
      ...
      <main className="min-h-screen bg-white">
        ...
        {(industry.services || []).length > 0 && (
          <section className="py-16 bg-white">
            <div className="max-w-[1400px] mx-auto px-4 sm:px-6">
              <div className="mb-12">
                {solutionsEyebrow && (
                  <span className="inline-block bg-[#00E1FF]/10 text-[#0055FF] text-sm font-semibold px-4 py-2 rounded-full mb-4">
                    {solutionsEyebrow}
                  </span>
                )}
                {solutionsHeading && (
                  <h2 className="text-2xl sm:text-3xl font-bold text-[#0f172a]">
                    {accentLastWord(solutionsHeading)}
                  </h2>
                )}
                {solutionsSub && solutionsSub !== solutionsHeading && (
            ...
            <div className="max-w-[1400px] mx-auto px-4 sm:px-6">
              {industry.challengesHeading && (
                <h2 className="text-2xl sm:text-3xl font-bold text-[#0f172a] mb-10">
                  {accentLastWord(industry.challengesHeading)}
                </h2>
              )}
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
                {industry.challenges.map((challenge, index) => (
                  <div
                    key={index}
                    className="bg-white rounded-xl p-6 border border-gray-100 hover:border-[#00E1FF]/40 hover:shadow-lg transition-all duration-300"
                  >
        ...
        {(industry.technologies || []).length > 0 && (
          <section className="py-16 bg-white">
            <div className="max-w-[1400px] mx-auto px-4 sm:px-6">
              {industry.technologiesHeading && (
                <h2 className="text-2xl sm:text-3xl font-bold text-[#0f172a] mb-8">
                  {accentLastWord(industry.technologiesHeading)}
                </h2>
              )}
              <div className="flex flex-wrap gap-3">
                {industry.technologies.map((tech) => (
                  <span
                    key={tech}
                    className="px-4 py-2 bg-[#f8f9fa] text-[#0f172a] text-sm font-medium rounded-full border border-gray-100"
                  >
                    {tech}
                  </span>
                ))}
              ...
              <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 items-center">
                <div className="lg:col-span-5">
                  <h2 className="text-3xl sm:text-4xl lg:text-5xl font-extrabold text-[#0f172a] mb-4">
                    {accentLastTwoWords(industry.whyChooseUsHeading || "")}
                  </h2>
                ...
                <div className="lg:col-span-7">
                  <div className="grid grid-cols-2 sm:grid-cols-3 gap-3.5 sm:gap-5">
                    {industry.whyChooseUsCards!.map((card, i) => {
                      const path = WHY_ICON_PATHS[card.icon] || WHY_ICON_PATHS.chart;
                      const lines = card.label.split("\n");
                      return (
                        <div
                          key={i}
                          className="bg-white rounded-2xl p-4 sm:p-6 flex flex-col items-center justify-center text-center shadow-sm hover:shadow-lg hover:border-[#00E1FF]/40 transition-all duration-300 min-h-[130px] sm:min-h-[160px] border border-gray-100 group"
                        >
                          <svg
                            className="w-7 h-7 sm:w-8 sm:h-8 text-[#0f172a] group-hover:text-[#00E1FF] transition-colors mb-3 sm:mb-4"
                            fill="none"
                            stroke="currentColor"
                            viewBox="0 0 24 24"
                            strokeWidth={1.75}
        ...
        <FAQ
          faqs={industry.faqs || []}
          eyebrow={industry.faqEyebrow}
          heading={industry.faqHeading}
          intro={industry.faqIntro}
        />
