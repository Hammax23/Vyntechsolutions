...
export default function AboutPageClient({
  ...
  initialChrome: AboutPageChrome;
  initialNavChrome?: typeof DEFAULT_NAV_CHROME;
  initialHomepage?: Record<string, unknown> | null;
}) {
  const [isVisible, setIsVisible] = useState(false);
  const [chrome] = useState(initialChrome);
  const [navChrome] = useState(initialNavChrome || DEFAULT_NAV_CHROME);
  ...
  const renderHeroHeading = () => {
    const heroHeading = chrome.heroHeading;
    ...
    if (heroHeading.includes("\n")) {
      ...
      return (
        <>
          {first}
          <span className="block mt-2 bg-gradient-to-r from-[#00E1FF] to-[#0055FF] text-transparent bg-clip-text">
            {restText}
          </span>
        </>
      );
  ...
  const missionStats = chrome.missionStats || [];
  const valuesList = chrome.values || [];
  const processList = chrome.process || [];
  const bodyHtml = chrome.bodyHtml?.trim() || "";
      ...
      <main className="min-h-screen bg-white">
        <section ref={heroRef} className="relative bg-[#0a0a14] pt-32 pb-24 overflow-hidden">
            ...
            {chrome.heroImageUrl && (
              <>
                <div
                  ...
                  style={{ backgroundImage: `url(${chrome.heroImageUrl})` }}
                />
          ...
          <div className="relative z-10 max-w-[1400px] mx-auto px-4 sm:px-6">
            <div
              className={`flex items-center gap-2 text-white/40 text-sm mb-12 transition-all duration-700 ${
            ...
            >
              ...
              <span className="text-white/70">{chrome.breadcrumbLabel}</span>
            </div>
        ...
        {bodyHtml ? (
          <section className="py-24 bg-gradient-to-b from-white to-gray-50">
            <div className="max-w-3xl mx-auto px-4 sm:px-6">
              <div className="bg-[#0a0a14] rounded-2xl p-8 sm:p-10 text-white/80">
                <CmsRichText
                  html={bodyHtml}
                  className="[&_h1]:text-white [&_h2]:text-white [&_h3]:text-white [&_p]:text-white/80 [&_li]:text-white/80 [&_strong]:text-white"
                />
              </div>
            </div>
          </section>
          ...
          <>
            {(chrome.missionHeading || chrome.missionBody || missionStats.length > 0) && (
              <section className="py-24 bg-gradient-to-b from-white to-gray-50">
                    ...
                    <div>
                      ...
                      {chrome.missionHeading ? (
                        <h2 className="text-3xl sm:text-4xl font-bold text-[#1a1a2e] mb-6 leading-tight">
                          {(() => {
                            const heading = chrome.missionHeading;
                            ...
                            if (words.length > 2) {
                              ...
                              return (
                                <>
                                  {words.join(" ")}{" "}
                                  <span className="bg-gradient-to-r from-[#00E1FF] to-[#0055FF] text-transparent bg-clip-text inline-block">
                                    {lastTwo}
                                  </span>
                                </>
                              );
            ...
            {(chrome.valuesEyebrow || chrome.valuesHeading || valuesList.length > 0) && (
              <section className="py-24 bg-[#0a0a14]">
                <div className="max-w-[1400px] mx-auto px-4 sm:px-6">
                  <div className="text-center mb-16">
                    {chrome.valuesEyebrow ? (
            ...
            {(chrome.processEyebrow || chrome.processHeading || processList.length > 0) && (
              <section className="py-24 bg-white">
                <div className="max-w-[1400px] mx-auto px-4 sm:px-6">
                  <div className="text-center mb-16">
                    {chrome.processEyebrow ? (
                      <span className="inline-block text-sm font-semibold bg-gradient-to-r from-[#00E1FF] to-[#0055FF] text-transparent bg-clip-text tracking-wider uppercase mb-4">
                        {chrome.processEyebrow}
                      </span>
                    ) : null}
                    ...
                    {chrome.processIntro ? (
                      <p className="text-gray-600 max-w-2xl mx-auto">{chrome.processIntro}</p>
                    ) : null}
                  </div>
        ...
        <TechnologyImpact initialHomepage={initialHomepage || null} />

        {(chrome.ctaHeading || chrome.ctaBody || chrome.ctaButtonLabel || chrome.ctaEmail) && (
              ...
              <div className="text-center max-w-3xl mx-auto">
                {chrome.ctaHeading ? (
                  <h2 className="text-3xl sm:text-4xl lg:text-5xl font-bold text-white mb-6">
                    {(() => {
                      const words = chrome.ctaHeading.split(" ");
                      if (words.length > 1) {
