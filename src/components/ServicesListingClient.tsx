...
export default function ServicesListingClient({
  ...
  initialList?: ListingService[];
  initialChrome?: ServicesListingDefaults;
  initialNavChrome?: typeof DEFAULT_NAV_CHROME;
}) {
  const [isVisible, setIsVisible] = useState(Boolean(initialList?.length));
  const [hoveredIndex, setHoveredIndex] = useState<number | null>(null);
  const [list, setList] = useState<ListingService[]>(
    initialList?.length ? initialList : listingFromData
  );
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      .catch(() => {});
  }, [initialList, initialChrome, initialNavChrome]);
