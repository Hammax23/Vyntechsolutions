...
function listingFromData(): ListingIndustry[] {
  return Object.entries(industriesData).map(([slug, industry]) => ({
    slug,
    title: industry.title,
    description: industry.description,
    icon: industry.icon || "healthcare",
...
function mergeListingChrome(
  ...
  return {
    ...
    heroCtaLabel:
      pick(page.heroCtaLabel, sections.heroCtaLabel) || defaults.heroCtaLabel,
    gridHeading:
      pick(sections.gridHeading, page.gridHeading) || defaults.gridHeading,
    ...
    stats,
    ctaHeading:
      pick(sections.ctaHeading, page.ctaHeading) || defaults.ctaHeading,
...
export default function IndustriesPage() {
  ...
  const [list, setList] = useState<ListingIndustry[]>(listingFromData);
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      fetch("/api/cms/content?type=static-page&slug=industries").then((r) =>
        r.ok ? r.json() : null
      ),
      ...
      .then(([industriesRes, staticRes, seoRes]) => {
        ...
        const merged = mergeListingChrome(industriesListingDefaults, page);
        setChrome(!cmsHero && seoH1 ? { ...merged, heroHeading: seoH1 } : merged);
        const cmsIndustries = industriesRes?.industries as
          | (Partial<IndustryData> & { slug?: string })[]
          | undefined;
          ...
          cmsIndustries
            .map((industry) => {
              ...
              return {
                ...
                description: String(
                  industry.description || fallback?.description || ""
                ),
        ...
        <section id="industries" className="py-20 bg-white">
          <div className="max-w-[1400px] mx-auto px-4 sm:px-6">
            <div className="text-center mb-16">
              ...
              <p className="text-gray-600 max-w-2xl mx-auto">{chrome.gridBody}</p>
            </div>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-8">
              {chrome.stats.map((stat, index) => (
                <div key={index} className="text-center">
                  <div className="text-4xl sm:text-5xl font-bold text-[#0d9488] mb-2">
                    {stat.value}
                  </div>
